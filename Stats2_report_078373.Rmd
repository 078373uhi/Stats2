...
```{r sentcorp2, echo=FALSE}
# add sentiments to previous text corpus
sentcorp  %>%
  inner_join(get_sentiments("bing")) %>%
  count(word, sentiment, sort = TRUE) %>%
  acast(word ~ sentiment, value.var = "n", fill = 0, scale = c(10, 0.1)) %>%
...
#### Sentiment Context
It is important to consider the context of a word when looking at the positivity value and when negation words are added it can be seen that a number of words have been wrongly assessed as negative so it may be that the corpus is not as negative as it initially appears; the wrongly assessed positive words though may negate some of this effect.
